...
class UDPSocket {
  public:
    ...
    bool Recieve(size_t *ip, uint16_t *port, std::string *data) {
        ...
        int data_size = recvfrom(socket_, buffer, MAX_MESSAGE_SIZE, MSG_DONTWAIT, (struct sockaddr *) &addr, &addrlen);
        if (data_size == -1) {
            if (errno != 22) {
                throw std::runtime_error(strerror(errno));
            } else {
...
int main() {
    ...
    int i = 20;
    while (i--) {
        client.Send((170524827), 5050, "google loh");
    }
    size_t ip;
    uint16_t port;
    std::string data;
    while (client.Recieve(&ip, &port, &data)) {
